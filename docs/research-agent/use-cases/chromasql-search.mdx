...
Query the SAP object index directly using ChromaSQL—without going through the Research Agent. This approach gives you precise control over search parameters and is ideal for bulk searches or when you know exactly what you're looking for.

ChromaSQL supports three search methods:

- **Keyword search**: Match exact phrases
- **Semantic search**: Match by meaning
- **Regex search**: Match patterns

You can also apply metadata filters such as `model_name = TCode` to search transaction codes only.

:::tip
For exploratory searches where you're unsure what you need, use [Find SAP Objects](find-sap-objects.md) with the Research Agent instead.
:::
...
- [List of objects](../capabilities/objects.md)
