...
### 2.2 SAP Restricted Subnet (No Inbound Ports)
...
### 2.3 LLM Embedding Service (Managed Cloud Service via Private Endpoint)
...
### 3.2 Adri AI Knowledge Graph Builder
...
### 4.1 Knowledge Graph Build Flow (Write Path)
...
### 4.3 What Crosses Network Boundaries
...
| VPC → LLM Service | ABAP content (scoped by customer configuration — see Section 3.5) | Via VPC endpoint (private network) | HTTPS over PrivateLink — no internet traversal                                    |
...
### 6.1 Firewall Between VPC and SAP Subnet
...
### 6.2 LLM Access via VPC Endpoints (Private Network)
...
### 6.3 Internet Access (Adri AI Server Only)
...
### 6.5 Connector-to-Server Authentication (mTLS + JWT)
